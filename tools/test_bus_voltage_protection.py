...
def main():
    ...
    fixture.write_text(PRELUDE+function_source((ROOT/'Foc/foc_sensing.c').read_text(),'Vbus_Update')+'\n'+function_source((ROOT/'Foc/foc_errhandle.c').read_text(),'ModeSwitch_Handle')+CASES)
    cc=[str(Path(a.cc).resolve())] if Path(a.cc).is_file() else [a.cc]
    if Path(a.cc).stem=='zig':cc+=['cc']
    exe=out/'bus_voltage_test.exe'
    command=cc+['-std=c99','-O1','-UNDEBUG','-Wall','-Wextra','-Werror','-I',str(out),'-I',str(ROOT/'System'),'-I',str(ROOT/'Foc'),str(fixture),str(ROOT/'software/config/motor_axis_profile.c'),'-o',str(exe)]
    logs=[]

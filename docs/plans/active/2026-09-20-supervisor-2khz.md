...
日期：2026-09-20。状态：**阶段 1–4 与状态机上移已实施**（PR 档全绿）；实机台架项（含 S5 CAN 回归）待授权；release 档待提交后执行。
...
- 延后：release 档（Keil 双目标、镜像/资源对比）；台架（心跳断连时延、温度超时与跳闸、
  CAN 节拍与波特率切换、bus-off 恢复）。
...
- 全部为 PR 档（离线）；release 档需先提交，台架项待授权。

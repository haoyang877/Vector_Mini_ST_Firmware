...
static bool Encoder_Calib_BuildLut(uint16_t minimum_samples)
{
    ...
    {
        int32_t correction =
            (int16_t)(p_error_sum[lut_index] / (int32_t)calibration_samples[lut_index]);
...
void Task_Calib_R_L_Flux(FOC_TypeDef *FOC, MotorControl_TypeDef *MotorControl)
{
    ...
    case CS_MOTOR_R_END:
    {
        ...
        MotorControl->motor_phase_resistance = (R_voltage[0] + R_voltage[1] + R_voltage[2]) /
    ...
    case CS_MOTOR_LD_LOOP:
    {
        ...
        else
        {
            if (++L_count <= 10000)
            {
                ...
                Is_testd += fast_sqrt(FOC->Ialpha_filt * FOC->Ialpha_filt +
                                      FOC->Ibeta_filt * FOC->Ibeta_filt);
            }
            else
            {
                loop_count = 0;
    ...
    case CS_MOTOR_LQ_LOOP:
    {
        ...
        else
        {
            if (++L_count <= 10000)
            {
                ...
                Is_testq += fast_sqrt(FOC->Ialpha_filt * FOC->Ialpha_filt +
                                      FOC->Ibeta_filt * FOC->Ibeta_filt);
            }
            else
            {
    ...
    {
        float Ld =
            (voltage_d - MotorControl->motor_phase_resistance * Is_testd) / (we_L * Is_testd);
        float Lq =
            (voltage_q - MotorControl->motor_phase_resistance * Is_testq) / (we_L * Is_testq);
        ...
        if (MotorControl->motor_d_inductance > 0.0005f ||
            MotorControl->motor_q_inductance > 0.0005f)
        {
    ...
    case CS_MOTOR_FLUX_END:
    {
        ...
        MotorControl->motor_flux =
            (voltage_norm - MotorControl->motor_phase_resistance * current_norm) / we_flux -
            L * current_norm;
                              ...
                              Fluxobserver_TypeDef *Fluxobserver)
{
    ...
    case CS_ENC_OFFSET_SAMPLE_CW:
        ...
        if (Encoder->read_status == ENCODER_READ_OK && theta_relative >= 0.0f &&
            theta_relative < required_theta)
        {
            uint16_t lut_index = Encoder->directed_q15 >> 6;
            uint16_t reference_q15 =
                (uint16_t)(theta_relative * ((float)ENCODER_Q15_CPR / required_theta));
        ...
        else if (time >= OBS_CALIB_TIMEOUT_FACTOR * required_theta / OBS_CALIB_SPEED +
                             OBS_CALIB_RAMP_TIME + OBS_CALIB_ALIGN_TIME)
        {
            Set_ErrorNow(Encoder_Error);
            Encoder_Calib_Abort();
        }

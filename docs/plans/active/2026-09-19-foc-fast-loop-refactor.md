...
- 旧 12 通道分析链（`tools/analysis/rtt_control_frame.py`、
  ...
  不再与当前固件配套；已于 2026-09-19 随 HIL 退役删除（通用解码器
  `tools/analysis/rtt_control_frame.py` 保留）。
...
- HIL 配套的 12 通道分析链已于 2026-09-19 随 HIL 退役删除（通用解码器
  `tools/analysis/rtt_control_frame.py` 保留），历史采集可继续用冻结输出回放。
- HIL 台架命令路径如需恢复，需要在 app 层重新引入邮箱轮询模块（HIL 已于 2026-09-19
  整体退役，恢复需另行立项）。

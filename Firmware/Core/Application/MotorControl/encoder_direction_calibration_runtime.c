#include "Core/Application/MotorControl/encoder_direction_calibration_runtime.h"
	...
	MotorStateContext *motor_state)
{
	...
	align_time_s = motor->commissioning_tuning->angle.direction_align_time_s;
	drive_speed_electrical_rad_s = motor->commissioning_tuning->angle.
		direction_speed_electrical_rad_s;
	align_current_a = motor->configuration.calibration_current_a;
	if (align_current_a < motor->commissioning_tuning->angle.
		electrical_zero_min_align_current_a)
		align_current_a = motor->commissioning_tuning->angle.
			electrical_zero_min_align_current_a;
	...
	time_s = (float)context->loop_count * motor->schedule.current_period_s;
	...
	if (context->state == ENCODER_DIRECTION_CALIBRATION_ALIGN)
	{
		motor->targets.d_axis_current_a = align_current_a *
			FastMath_Clamp(((float)context->loop_count + 1.0f) *
				motor->schedule.current_period_s / align_time_s, 0.0f, 1.0f);
	...
	if (context->state == ENCODER_DIRECTION_CALIBRATION_ROTATE)
	{
		...
		context->drive_phase_rad += drive_speed_electrical_rad_s *
			motor->schedule.current_period_s;
		motor->targets.d_axis_current_a = align_current_a;
		motor->targets.q_axis_current_a = 0.0f;

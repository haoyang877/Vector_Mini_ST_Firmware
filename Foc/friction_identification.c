...
static float MaxValue(float first, float second)
{
	return first >= second ? first : second;
}

static bool IsActive(const FrictionIdentification_TypeDef *identification)
{
...
static bool ConfigIsValid(const FrictionIdentificationConfig_TypeDef *config)
{
	uint32_t index;
	...
	if (config == NULL || !isfinite(config->update_period_s) ||
		...
		!isfinite(config->speed_tolerance_ratio) ||
		config->speed_tolerance_ratio <= 0.0f ||
		...
		!isfinite(config->minimum_sample_time_s) ||
		config->minimum_sample_time_s <= 0.0f ||
...
static uint32_t SecondsToTicks(const FrictionIdentification_TypeDef *identification,
	float seconds)
{
	float ticks = seconds / identification->config.update_period_s;

	if (!isfinite(ticks) || ticks <= 1.0f)
		return 1U;
	if (ticks >= 4294967040.0f)
		return UINT32_MAX;
	return (uint32_t)(ticks + 0.5f);
}

static float TargetForPoint(const FrictionIdentification_TypeDef *identification,
...
static float SpeedTolerance(const FrictionIdentification_TypeDef *identification,
	float target_speed)
{
	return MaxValue(AbsValue(target_speed) *
		identification->config.speed_tolerance_ratio,
		identification->config.minimum_speed_tolerance_rad_s);
}

static bool FitDirection(FrictionIdentification_TypeDef *identification, bool positive,
	float *coulomb_a, float *viscous_a_per_rad_s, float *rmse_a)
{
	...
	if (!isfinite(denominator) || denominator <= 1e-9f)
		return false;
	...
	{
		intercept = 0.0f;
		slope = sum_xx > 0.0f ? sum_xy / sum_xx : 0.0f;
	}
	...
	*rmse_a = sqrtf(error_squared_sum / (float)count);
	...
	if (!isfinite(*rmse_a) || *rmse_a > MaxValue(
		identification->config.rmse_floor_a,
...
static bool Fit(FrictionIdentification_TypeDef *identification)
{
...
void FrictionIdentification_Reset(FrictionIdentification_TypeDef *identification)
{
	...
	memset(identification, 0, sizeof(*identification));
	identification->config = config;
	identification->state = ConfigIsValid(&config) ?
		FRICTION_IDENT_IDLE : FRICTION_IDENT_FAILED;
	...
	const FrictionIdentificationInput_TypeDef *input)
{
	float target_speed;
	float tolerance;
	uint32_t minimum_sample_ticks;
	uint32_t total_sample_count;

	if (identification == NULL || input == NULL || !IsActive(identification))
		return;
	if (!isfinite(input->measured_speed_rad_s) ||
		!isfinite(input->ramped_speed_reference_rad_s) ||
		!isfinite(input->iq_a) || !isfinite(input->mechanical_position_rad))
	{
		FrictionIdentification_Fail(identification,
	...
	{
		if (AbsValue(input->measured_speed_rad_s) <=
			identification->config.stop_speed_rad_s &&
			AbsValue(input->ramped_speed_reference_rad_s) <=
			identification->config.stop_speed_rad_s)
			...
			identification->config.stop_hold_time_s))
		{
			...
			identification->state = FRICTION_IDENT_TRACKING;
			identification->target_speed_rad_s =
				TargetForPoint(identification, identification->point_index);
			return;
		}
	...
	{
		/* Encoder quantisation, cogging and low-speed PI ripple make a
		 * consecutive-sample band test unnecessarily brittle. Validate the
		 * average of one complete dwell window instead. */
		...
		{
			identification->stable_ticks = 0U;
			identification->tracking_mean_speed = 0.0f;
		}
		if (identification->stable_ticks >= SecondsToTicks(identification,
			identification->config.stable_time_s))
		{
			if (AbsValue(identification->tracking_mean_speed - target_speed) <= tolerance)
			{
				...
				identification->sample_start_position =
					input->mechanical_position_rad;
			}
			else
			{
				identification->stable_ticks = 0U;
	...
	/* Average every sample over the complete mechanical revolution. The
	 * measured mean, rather than the command, is used by the regression. */
	...
	if (identification->sample_count >= minimum_sample_ticks &&
		AbsValue(input->mechanical_position_rad -
			identification->sample_start_position) >=
			identification->config.sample_turns * FRICTION_TWO_PI)
	{
		FrictionIdentificationSample_TypeDef *sample =
	...
	const FrictionIdentification_TypeDef *identification)
{
	...
	if (identification == NULL || identification->state == FRICTION_IDENT_IDLE ||
		identification->state == FRICTION_IDENT_FAILED)
		return 0.0f;
	if (identification->state == FRICTION_IDENT_COMPLETE)
		return 100.0f;
...
FrictionIdentificationReason_TypeDef FrictionIdentification_GetReason(
	const FrictionIdentification_TypeDef *identification)
{
	return identification != NULL ? identification->reason :
		FRICTION_IDENT_REASON_INVALID_CONFIG;
}

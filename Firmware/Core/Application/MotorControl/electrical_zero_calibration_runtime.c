#include "Core/Application/MotorControl/electrical_zero_calibration_runtime.h"
	...
	EncoderContext *encoder, MotorStateContext *motor_state)
{
	...
	ramp_time_s = motor->commissioning_tuning->angle.
		electrical_zero_current_ramp_time_s;
	hold_time_s = motor->commissioning_tuning->angle.
		electrical_zero_hold_time_s;
	align_time_s = ramp_time_s + hold_time_s;
	time = (float)context->loop_count * motor->schedule.current_period_s;
	align_current = motor->configuration.calibration_current_a;
	if (align_current < motor->commissioning_tuning->angle.
		electrical_zero_min_align_current_a)
		align_current = motor->commissioning_tuning->angle.
			electrical_zero_min_align_current_a;
	...
	motor->targets.d_axis_current_a = align_current * FastMath_Clamp(
		((float)context->loop_count + 1.0f) * motor->schedule.current_period_s /

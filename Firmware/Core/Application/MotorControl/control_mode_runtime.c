#include "Core/Application/MotorControl/control_mode_runtime.h"
	...
	FluxObserverContext *Fluxobserver, const RotorFeedbackFrame *feedback)
{
	...
	if (feedback != 0)
	{
		theta_elec = feedback->electrical_angle_rad;
		vel_elec = feedback->electrical_velocity_rad_s;
	}
	else if(MotorControl->configuration.use_sensorless_feedback == true)
	{
	...
	EncoderContext *Encoder, const RotorFeedbackFrame *feedback)
{
	...
	bool use_speed_ramp;
	    ...
	    if (feedback != 0)
	    {
	        theta_elec = feedback->electrical_angle_rad;
	        vel_elec = feedback->electrical_velocity_rad_s;
	        vel_mech = feedback->motor_velocity_rad_s;
	    }
	    else
	    {
	        theta_elec = Encoder_GetElePhase(Encoder);
    ...
    if (++motion->speed_loop_count >= MotorControl->schedule.speed_divider)
    {
        ...
        if (use_speed_ramp)
        {
            ...
            {
                MotorControl->runtime.speed_command_ramp_rad_s += MotorControl->configuration.speed_acceleration_rad_s2 * MotorControl->schedule.speed_period_s;
            ...
            {
                MotorControl->runtime.speed_command_ramp_rad_s -= MotorControl->configuration.speed_deceleration_rad_s2 * MotorControl->schedule.speed_period_s;
                if (MotorControl->runtime.speed_command_ramp_rad_s < MotorControl->targets.speed_rad_s)
                {
                    MotorControl->runtime.speed_command_ramp_rad_s = MotorControl->targets.speed_rad_s;
                }
        ...
        PI_Controller_Configure(controller, MotorControl->configuration.speed_kp, MotorControl->configuration.speed_ki, MotorControl->schedule.speed_period_s, -1.0f, 1.0f);
...
static bool Sensorless_StartupTuningIsValid(
	const ProductSensorlessStartupConfig *tuning)
{
						...
						MotorStateContext *motor_state)
{
	...
	{
		FluxObserver_Initialize(Fluxobserver, &MotorControl->control_config->sensorless,
			MotorControl);
	...
	switch (Startup->state)
	{
		case SENSORLESS_STARTUP_ALIGN:
			MotorControl->targets.d_axis_current_a = tuning->align_current_a *
				FastMath_Clamp(((float)Startup->state_ticks + 1.0f) * MotorControl->schedule.current_period_s /
					tuning->align_current_ramp_time_s, 0.0f, 1.0f);
			MotorControl->targets.q_axis_current_a = 0.0f;
			CurrentControlRuntime_RunClosedLoop(CurrentControl, MotorControl, 0.0f, 0.0f);
			if (++Startup->state_ticks >= (uint32_t)((tuning->align_current_ramp_time_s +
				tuning->align_hold_time_s) / MotorControl->schedule.current_period_s))
			{
		...
		case SENSORLESS_STARTUP_OPEN_LOOP:
		{
				...
				tuning->target_electrical_velocity_rad_s)
			{
				Startup->open_loop_omega += Startup->direction *
					(tuning->target_electrical_velocity_rad_s /
					 tuning->startup_ramp_time_s) * MotorControl->schedule.current_period_s;
				if (FastMath_Abs(Startup->open_loop_omega) >=
					tuning->target_electrical_velocity_rad_s)
					Startup->open_loop_omega = Startup->direction *
						tuning->target_electrical_velocity_rad_s;
			...
			iq_ramp_ratio = FastMath_Clamp((float)Startup->open_loop_ticks *
				MotorControl->schedule.current_period_s / tuning->startup_iq_ramp_time_s, 0.0f, 1.0f);

			Startup->open_loop_theta = FastMath_NormalizeAngle(Startup->open_loop_theta + Startup->open_loop_omega * MotorControl->schedule.current_period_s);
		...
		if (requested_direction != Startup->direction ||
				...
				MotorControl->schedule.current_period_s))
			{
				...
				PI_Controller_Configure(controller, MotorControl->configuration.speed_kp, MotorControl->configuration.speed_ki,
					MotorControl->schedule.speed_period_s, -1.0f, Startup->speed_pi_output_max);
		...
		case SENSORLESS_STARTUP_HANDOFF:
		{
			...
			blend = FastMath_Clamp((float)(++Startup->state_ticks) *
				MotorControl->schedule.current_period_s / tuning->angle_handoff_time_s, 0.0f, 1.0f);
		...
		case SENSORLESS_STARTUP_CLOSED_LOOP:
		{
			...
			MotorControl->targets.d_axis_current_a = tuning->startup_id_a *
				...
				MotorControl->schedule.current_period_s / tuning->id_ramp_down_time_s, 0.0f, 1.0f));
			if (Startup->id_ramp_ticks < (uint32_t)(tuning->id_ramp_down_time_s /
				MotorControl->schedule.current_period_s))
				Startup->id_ramp_ticks++;

			if (++Startup->speed_loop_ticks >= MotorControl->schedule.speed_divider)
			{
	...
	MotorStateContext *motor_state)
{
	...
	if (motion == 0 || CurrentControl == 0 || MotorControl == 0 || Encoder == 0 ||
		MotorControl->control_config == 0)
		return;
	if (feedback != 0)
	{
		theta_elec = feedback->electrical_angle_rad;
		theta_mech = feedback->control_position_rad;
		vel_elec = feedback->electrical_velocity_rad_s;
		vel_mech = feedback->control_velocity_rad_s;
	}
	else
	{
		theta_elec = Encoder_GetElePhase(Encoder);
	...
	config.position_kp_limit = MotorControl->control_config->parameter_limits.
		cascade_position_kp_limit_per_s;
	config.position_kd_limit = MotorControl->control_config->parameter_limits.
		cascade_position_kd_limit;
	config.position_sample_period_s =
		MotorControl->schedule.cascade_position_period_s;
	config.speed_sample_period_s = MotorControl->schedule.speed_period_s;
	config.position_loop_divider =
		MotorControl->schedule.cascade_position_divider;
	config.speed_loop_divider = MotorControl->schedule.speed_divider;
	...
	MotorStateContext *motor_state)
{
	...
	if (motion == 0 || CurrentControl == 0 || MotorControl == 0 || Encoder == 0 ||
		MotorControl->control_config == 0 || !isfinite(maximum_current_limit_a) ||
	...
	config.kp_limit = MotorControl->control_config->parameter_limits.
		position_kp_limit_a_per_rad;
	config.kd_limit = MotorControl->control_config->parameter_limits.
		position_kd_limit_a_per_rad_s;
	config.ki_limit = MotorControl->control_config->parameter_limits.
		position_ki_limit_a_per_rad_s;
	config.maximum_speed_limit_rad_s = MotorControl->control_config->
	...
	config.friction_feedforward_enabled = MotorControl->configuration.
		friction_model_valid || MotorControl->control_config->position_friction.enabled;
	config.friction_positive_current = MotorControl->configuration.
		...
		MotorControl->control_config->position_friction.friction_positive_current_a;
	config.friction_negative_current = MotorControl->configuration.
		...
		MotorControl->control_config->position_friction.friction_negative_current_a;
	config.breakaway_positive_current = FastMath_Max(
		MotorControl->control_config->position_friction.breakaway_positive_current_a,
		config.friction_positive_current);
	config.breakaway_negative_current = FastMath_Max(
		MotorControl->control_config->position_friction.breakaway_negative_current_a,
		config.friction_negative_current);
	config.friction_current_slew_rate = MotorControl->control_config->
		position_friction.current_slew_rate_a_per_s;
	config.friction_position_enter = MotorControl->control_config->
		position_friction.position_enter_rad;
	config.friction_position_exit = MotorControl->control_config->
		position_friction.position_exit_rad;
	config.friction_reference_speed = MotorControl->control_config->
		position_friction.reference_speed_rad_s;
	config.friction_stop_speed = MotorControl->control_config->
		position_friction.stop_speed_rad_s;
	config.friction_move_speed = MotorControl->control_config->
		position_friction.move_speed_rad_s;
	config.friction_stuck_time = MotorControl->control_config->
		position_friction.stuck_time_s;
	config.friction_landing_position = MotorControl->control_config->
		position_friction.landing_position_rad;
	config.friction_landing_speed = MotorControl->control_config->
		position_friction.landing_speed_rad_s;
	config.friction_recovery_delay = MotorControl->control_config->
		position_friction.recovery_delay_s;
	config.friction_recovery_pulse_time = MotorControl->control_config->
		position_friction.recovery_pulse_time_s;
	config.friction_recovery_cooldown = MotorControl->control_config->
		position_friction.recovery_cooldown_s;
	config.sample_period_s = MotorControl->schedule.position_period_s;
	config.loop_divider = MotorControl->schedule.position_divider;
...
void ControlModeRuntime_RunVoltageOpenLoop(CurrentControlContext *CurrentControl, MotorControlContext *MotorControl)
{
	...
	MotorControl->runtime.open_loop_electrical_angle_rad = FastMath_NormalizeAngle(MotorControl->runtime.open_loop_electrical_angle_rad + MotorControl->configuration.open_loop_electrical_velocity_rad_s * MotorControl->schedule.current_period_s);
...
void ControlModeRuntime_RunQVoltage(CurrentControlContext *CurrentControl,
	...
	MotorStateContext *motor_state)
{
	if (feedback == 0 && !Encoder_IsOnline(Encoder))
	{
	...
	CurrentControlRuntime_RunQVoltage(CurrentControl,
				MotorControl,
				feedback != 0 ? feedback->electrical_angle_rad :
					Encoder_GetElePhase(Encoder),
				feedback != 0 ? feedback->electrical_velocity_rad_s :

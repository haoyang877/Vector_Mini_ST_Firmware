#include "Core/Application/MotorControl/current_control_runtime.h"
	...
	const MotorControlContext *MotorControl)
{
	...
	CurrentControl->id_pi.Ts = MotorControl->schedule.current_period_s;
	...
	CurrentControl->iq_pi.Ts = MotorControl->schedule.current_period_s;
}
...
static void CurrentControlRuntime_ApplyModulation(CurrentControlContext *CurrentControl)
{
	...
	if (!Svpwm_Calculate(CurrentControl->alpha_modulation, CurrentControl->beta_modulation, &output))
	{
		...
		MotorDriveService_RejectOutputCommand(CurrentControl->motor_drive);
		return;
	}
	...
	(void)MotorDriveService_ApplyFixedDuty(CurrentControl->motor_drive,
...
void CurrentControlRuntime_RunClosedLoop(CurrentControlContext *CurrentControl, MotorControlContext *MotorControl, float phase, float phase_vel)
{
    ...
    CurrentControlRuntime_InversePark(CurrentControl->d_axis_modulation, CurrentControl->q_axis_modulation,
        phase + phase_vel * MotorControl->schedule.current_period_s, &CurrentControl->alpha_modulation, &CurrentControl->beta_modulation);

...
| 本次状态 | 已写入Flash并复位读回 | 配置草稿；未连接确认、未写入、未运动测试 |
| 配置文件 | [roll.json](../tests/mode3/roll.json) | [pitch.json](../tests/mode3/pitch.json) |

角度沿用各电机自身编码器机械零点。型号与硬件相同不表示编码器零点、偏心补偿或装配摩擦相同；pitch不会自动复制roll的校准区。pitch运行增益在模板中保留null，待实际读回和验证。
...
实机测试后端现在支持 `--axis-profile tests/mode3/roll.json` 或pitch配置：先核对运行上下文的AXS1名称/CRC/范围/速度、实际电流上限与加减速度，再使能。目标与主机停止边界同时收窄；pitch采用5°留量时，目标在±25°内，主机停止边界±28°，模式3持久化边界±30°。HIL原±90°保护仍作为外层限制，不能用它代替pitch的窄范围约束。
...
roll提交完成后，再确认切换到pitch。先只读核对pitch实际固件、校准与零点，配置持久化±30°边界及测试主机±28°停止范围；完成必要校准后，从较小目标开始。未确认物理接线和校准前，不以roll数据推断pitch已可运动。

...
{
	float minimum_mechanical_speed = MotorControl->mechanical_load_profile->
		encoder_calibration_startup.minimum_electrical_velocity_rad_s /
		(float)MotorControl->configuration.pole_pairs;
	...
	EncoderContext *Encoder, MotorStateContext *motor_state)
{
	...
	switch (context->step)
	{
		...
		case CS_ENC_OFFSET_SAMPLE_CW:
			...
			context->encoder_linearization.drive_phase += context->encoder_linearization.drive_omega * CURRENT_LOOP_PERIOD_S;
	...
	MotorStateContext *motor_state)
{
	...
	if (context->step == CS_OBS_STOP_CURRENT)
	{
		...
		current_ratio = FastMath_Clamp(((float)context->observer_calibration.state_ticks + 1.0f) * CURRENT_LOOP_PERIOD_S /
	...
	else
	{
		...
		if (context->step == CS_OBS_STOP_DECEL)
		{
			float decel_ratio = FastMath_Clamp((float)context->observer_calibration.state_ticks * CURRENT_LOOP_PERIOD_S /
	...
	switch (context->step)
	{
		case CS_OBS_ALIGN_ORIGIN:
			if (Startup->state == SENSORLESS_STARTUP_ALIGN &&
				Startup->state_ticks >= (uint32_t)(((MotorControl->mechanical_load_profile->
					encoder_calibration_startup.align_current_ramp_time_s +
					MotorControl->mechanical_load_profile->encoder_calibration_startup.align_hold_time_s) -
					SENSORLESS_ENCODER_CALIB_ALIGN_SAMPLE_TIME_S) / CURRENT_LOOP_PERIOD_S))
			{
		...
		case CS_OBS_SPEED_STABLE:
			...
			{
				if (++context->observer_calibration.state_ticks >= (uint32_t)(SENSORLESS_ENCODER_CALIB_SPEED_STABLE_TIME_S / CURRENT_LOOP_PERIOD_S))
				{
		...
		case CS_OBS_STOP_DECEL:
		{
			...
			if (context->observer_calibration.state_ticks >= (uint32_t)(SENSORLESS_ENCODER_CALIB_STOP_DECEL_TIME_S /
				CURRENT_LOOP_PERIOD_S) && speed_reached)
			{

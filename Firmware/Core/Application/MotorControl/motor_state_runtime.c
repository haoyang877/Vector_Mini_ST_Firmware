#include "Core/Application/MotorControl/motor_state_runtime.h"
...
static bool MotorLifecycle_TryGetControlPosition(
	const MotorStateContext *context, float *position_rad)
{
	const RotorFeedbackFrame *feedback = RuntimeBindings.feedback;
	FeedbackRouterSignal required_signal;

	if (position_rad == NULL)
		return false;
	if (feedback == NULL)
	{
		*position_rad = Encoder_GetMecPos(&OnBoard_Encoder);
		return isfinite(*position_rad);
	}
	required_signal = feedback->uses_output_position ?
		FEEDBACK_ROUTER_SIGNAL_OUTPUT_POSITION :
		FEEDBACK_ROUTER_SIGNAL_MOTOR_POSITION;
	if ((feedback->valid_signals &
		FEEDBACK_ROUTER_SIGNAL_MASK(required_signal)) == 0U ||
		!isfinite(feedback->control_position_rad))
	{
		return false;
	}
	*position_rad = feedback->control_position_rad;
	return true;
}
	...
	MotorStateContext *context, MotorControlMode mode)
{
	...
	/* A disabled product feature is a rejected command, not a motor fault. */
	if (!MotorControlModeMask_Allows(State.allowed_control_mode_mask, mode) ||
	...
	if ((mode == MOTOR_CONTROL_MODE_POSITION_CASCADE ||
		 mode == MOTOR_CONTROL_MODE_POSITION_IMPEDANCE) &&
		!MotorLifecycle_TryGetControlPosition(context, &position_rad))
	{
		MotorState_RaiseFault(context, MOTOR_FAULT_ENCODER);
		return false;
	}
	...
	MotorControlModeMask allowed_control_mode_mask)
{
	if (context == NULL || bindings == NULL || bindings->motor == NULL ||
		...
		allowed_control_mode_mask == 0U ||
		(allowed_control_mode_mask & ~MOTOR_CONTROL_SUPPORTED_MODE_MASK) != 0U)
		return false;
	...
	MotorStateChanged = true;
	State.allowed_control_mode_mask = allowed_control_mode_mask;
	MotorCommissioningWorkflow_Initialize(&State.commissioning);
	if (!MotorCommissioningWorkflow_Configure(&State.commissioning,
			commissioning_stage_mask))
	{
		return false;
	}
	...
	MotorControlMode mode)
{
		...
		mode == MOTOR_CONTROL_MODE_POSITION_IMPEDANCE)
	{
		...
		if (!MotorLifecycle_TryGetControlPosition(context, &current_position))
		{
			MotorState_RaiseFault(context, MOTOR_FAULT_ENCODER);
			return false;
		}
	...
	ServiceProcedure procedure)
{
	...
	if (context == NULL ||
		!MotorCommissioningWorkflow_IsServiceEnabled(&State.commissioning,
			procedure) ||
		!MotorLifecycle_CheckServicePreconditions(context, procedure))
		return false;
	...
	uint32_t request)
{
	...
	switch (type)
	{
		...
		case LIFECYCLE_REQUEST_CLEAR_FAULTS:
			(void)MotorDriveService_ClearLatchedFault(
				CurrentControl.motor_drive);
	...
	if (MotorLifecycle.device_state == DEVICE_STATE_SERVICING)
	{
			...
			result == SERVICE_RESULT_COMPLETE_AND_SAVE)
		{
					...
					&commissioning_complete))
			{
				...
				else if (MotorLifecycle_CheckServicePreconditions(context,
						next_procedure))
					PendingLifecycleRequest = LIFECYCLE_REQUEST_ENCODE(
...
{
	(void)context;
	MotorDriveService_ForceDisable(CurrentControl.motor_drive);
}
bool MotorState_EnableMotorDrive(MotorStateContext *context)
{
	return MotorDriveService_RequestEnable(CurrentControl.motor_drive,
		!MotorFaults_HasActive(context));
}

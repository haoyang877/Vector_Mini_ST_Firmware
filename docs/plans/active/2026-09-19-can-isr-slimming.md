...
**2026-09-20：已按方案 A 实施**（全部命令统一入队，不保留中断内快速路径）；
实现记录与证据见 §10。

> **2026-09-20 修订**：监督时基已由 1 kHz 提升为 2 kHz（[计划](2026-09-20-supervisor-2khz.md)）。节拍相关时延上限减半（应答 ≤0.5 ms，命令生效最坏增加一个 2 kHz 周期）；ms 级超时与状态流相位语义不变。

## 1. 意图

按用户给出的通信时序（2026-09-19 澄清）重构收发路径：**中断只做有界采集 → 2 kHz 时基
派发与组包 → 2 kHz 时基发送**。

```text
FDCAN1_IT0 (prio 1)  取帧 → 帧级过滤 → 解码(≤4B) → 刷新心跳 → 入 RX ring
TIM7 2 kHz (prio 3)  ① 排空 RX ring：逐条派发（写路径/读路径，结果入 TX ring）
                     ② 排空 TX ring：发应答；空闲时按周期发 48 B 状态流
主循环               不再承担 CAN 收发（保留 FocCogging_Service 与保存会话）
```

本阶段（S5）的**核心**是把 RX 派发移出 CAN 中断；**TX ring 与 2 kHz 发送节拍**属同一次
...
| 事实 | 证据 |
| --- | --- |
| 中断优先级（数值越小越高）：`ADC1_2`(20 kHz FOC)=0、`FDCAN1_IT0`(CAN RX)=1、`TIM7`(2 kHz)=3、`DMA1_Channel1`=5 | `firmware/platform/stm32g4/cubemx/Core/Src/fdcan.c:104`、`adc.c:271,317`、`tim.c:263`、`dma.c:48` |
...
| 2 kHz tick 现有内容：`FOC2kHzSupervisor` + LED(每 400 拍) + RGB(每 100 拍) + `CAN_BaudRateSwitching`(每 200 拍) + `CAN_DisConnect_Handle`(每拍)，由 TIM7 中断（prio 3）调用 | `firmware/app/bsp_task.c:46-69`、`stm32g4xx_it.c:360-365` |
...
```text
FDCAN1_IT0 (prio 1)                            TIM7 2 kHz (prio 3)
...
- **RX ring**：单生产者（CAN ISR）/单消费者（2 kHz tick）定长环，元素 `{CAN_PARAM_ID, float}`；
  `volatile` 头尾索引；无锁、不等待、不分配；满则丢弃最新并计数。
- **CAN 中断侧**：保持取帧/帧级过滤/解码/心跳刷新，仅把"派发"替换为"入队"；工作量有界。
- **TX ring**：生产者是 2 kHz 里的派发结果（`CAN_SendMessage_Update`），消费者是同一 tick 的
  发送段；FIFO 顺序；**应答优先于状态流**（保持现状"reply first"语义）；满则丢弃最新并计数。
- **发送节拍**：由主循环改为 2 kHz 固定节拍；状态流仍走 `CanMotorStatus_Prepare` 的 1 ms
  相位累积与"队列空闲才发"约束（`comm_hw_can_try_send_status`），不再依赖主循环迭代频率。
- **临界区**：2 kHz tick 在 prio 3，20 kHz 快环（prio 0）可抢占它，因此派发仍需**每命令短临界区**
...
1. 命令生效时刻（控制相位）由"CAN 中断即时"变为"2 kHz 拍沿"，最坏增加一个 2 kHz 周期；
2. **应答发送时机**由主循环改为 2 kHz（上位机看到的应答时延变为 ≤0.5 ms + 临界区/总线时间）；
3. **突发应答丢失被修复**：现状单槽覆盖会丢掉前一条应答，TX ring 后按 FIFO 全部发出
   （这是**行为变化**，需在回归中明确验证为改善且不破坏顺序/编号）；
4. 新增 RX/TX 队列满的丢帧路径与计数（有界、可观测、不静默）；
5. CAN 中断时长下降；2 kHz tick 与主循环单次迭代时长变化；
...
- 新增原生夹具（TX）：应答 FIFO 顺序、**突发多条查询不再丢应答**（现状单槽会丢，作为回归基线）、
  应答优先于状态流、TX 队满计数；
- 2 kHz 节拍：在夹具中以显式 tick 驱动，验证状态流按 1 ms 相位累积与 `CanMotorStatus_Prepare`
  的既有速率语义不变；
...
| D5-b | 派发消费点 | **已按用户规格更正**：2 kHz 时基（TIM7 tick，prio 3），不再放主循环入口。 |
...
| D5-a | 是否保留中断内快速路径 | **已裁决（2026-09-20）：方案 A——全部命令统一入队，不保留中断内快速路径。** |
...
**D5-e**：TX ring 与 2 kHz 发送节拍是否与 RX 一起纳入本阶段？（用户 2026-09-19 规格已包含，
...
组包放在 2 kHz 中断内（有界、定节拍）。用户规格明确接受该形态；需在实施时同步更新
`communication_layering.md` 的偏差记录。

## 9. 证据与状态

- 已于 2026-09-20 按方案 A 实施并离线验证；实现记录与证据见 §10。
...
- 实施入口（按用户规格更正）：
  ...
  `CAN_SendMessage_Update` 语义调整 + `CAN_GetRxDropCount/CAN_GetTxDropCount` 契约 + 2 kHz 服务入口）、
  `bsp_task.c`（2 kHz tick 内追加 RX/TX 排空）、`main.c`（移除 `CAN_SendMessage()` 调用）、
  夹具 `run_can_status_tests.py`（RX/TX 队列与突发应答用例）与差分夹具。
- 用户规格澄清（2026-09-19）：RX 中断只入队；2 kHz tick 排空 RX、派发、组包入 TX ring 并发应答；
  发送同样按 2 kHz 节拍。已据此把 D5-b 由"主循环入口"更正为"2 kHz 时基"，并把 TX ring 纳入范围。

## 10. 实施记录（2026-09-20，方案 A）

- 决策：D5-a 选择“全部命令统一入队”，不保留中断内快速路径。
- 实现：`can_transport.{c,h}` 新增 RX/TX 定长环（各 8 条，满则丢弃最新并计数；
  RX 单生产者/单消费者，TX 单上下文）；`can_command_binding.c` 中断只解码入队，新增
  `CanCommand_ServiceRx()` 排空派发；越界齿槽索引由中断与派发共用的判定在入队前拒绝；
  `interface_can.{c,h}` 新增 `CAN_Service()`（RX 派发 → 应答 FIFO → 状态流）与
  `CAN_GetRxDropCount/CAN_GetTxDropCount`，`CAN_SendMessage_Update` 改为应答入队，
  删除 `CAN_SendMessage()` 与单槽发送字段；`bsp_task.c` 2 kHz tick 首步调用 `CAN_Service()`；
  `main.c` 移除发送调用。
- 实施细化（相对 §4 设计）：① 短临界区只包写路径（恢复“命令对快环原子”）；读路径不屏蔽
  中断，因为 COGGING 查询含 2 KB 表签名 CRC，屏蔽会超出 50 µs 快环预算；② 状态流在
  “本拍存在任何待发应答（含本拍派发产生）”时不提交，保持逐拍 reply-first；
  ③ 断连故障清除仍由运行状态机恢复矩阵执行。
- 验证（离线，PR 档全绿）：`outputs/runs/20260919T175256075981Z-90982812/summary.json`；
  `run_can_status_tests` 覆盖 RX 心跳/解码/守卫/越界索引不入队/FIFO 顺序/环满计数、
  发送服务应答优先与 TX 环计数、应答编码、心跳超时、48 B 金帧与 HAL 端口。
- 实机验收（2026-09-20，node 1 台架；证据 `outputs/bench_20260920/verify_refactor_result.json`）：
  ① 只读预检与限值查询通过；② 突发 5 条 GET_MODE 应答 5/5（旧单槽只会保留最后一条，
  缺陷确认修复）；③ 心跳 500 ms 下主机停发帧 0.404 s 内退出运动模式并关断，链路恢复后
  命令链可用；④ 模式 2/3 闭环与 S5 队列派发同链路验证通过。
- 未重复执行：波特率切换与 bus-off 恢复（本次未改实现与节拍，2026-09-19 已台架验证）；
  RX 环满丢帧计数仅 C API 可读，未做主机侧注入（FIFO 行为由原生夹具覆盖）。

	...
	MotorParameterId parameter, float value)
{
	return UsbCommandRouter_MapParameterResult(
		ParameterService_WriteMotorParameter(parameter, value));
}

static float UsbCommandRouter_ReadParameter(MotorParameterId parameter,
	float scale)
{
	float value = 0.0f;
	(void)ParameterService_ReadMotorParameter(parameter, &value);
	return value * scale;
}

static float UsbCommandRouter_ReadTelemetry(MotorTelemetryId telemetry,
	float scale)
{
	float value = 0.0f;
	(void)TelemetryService_ReadValue(telemetry, &value);
	return value * scale;
}

static UsbCommandError UsbCommandRouter_Write(UsbParameterId parameter,
	float value, uint8_t value_is_float)
{
	...
	switch (parameter)
	{
		case USB_MODE:
			...
			if (integer_value < 0 || integer_value >= USB_COMMAND_ROUTER_MAX_MODE_VALUE)
				return USB_DATA_OUT_OF_RANGE;
			return MotorCommandService_RequestActionCode((uint8_t)integer_value) ==
				MOTOR_COMMAND_ACCEPTED ? USB_NO_ERROR : USB_WRITE_INVALID;
		case USB_CURRENT_SET:
			command_result = MotorCommandService_SetCurrentReferenceA(value);
			break;
		case USB_SPEED_SET:
			command_result = MotorCommandService_SetSpeedReferenceRps(value);
			break;
		case USB_POS_SET:
			command_result = MotorCommandService_SetPositionReferenceRevolutions(value);
			break;
		case USB_NODE_ID:
			if (value_is_float != 0U)
				return USB_DATA_INVALID;
			return integer_value >= 0 && integer_value <= 7 &&
				CanConfigurationService_SetNodeId((uint8_t)integer_value) ?
				USB_NO_ERROR : USB_DATA_OUT_OF_RANGE;
		case USB_POLEPARIS:
			if (value_is_float != 0U)
				return USB_DATA_INVALID;
			return UsbCommandRouter_WriteParameter(MOTOR_PARAMETER_POLE_PAIRS, value);
		case USB_ENCODER_STATE:
			return value_is_float != 0U ? USB_DATA_INVALID : USB_DATA_OUT_OF_RANGE;
		case USB_ENCODER_REVERSE:
			if (value_is_float != 0U)
				return USB_DATA_INVALID;
			if ((int)UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_MODE, 1.0f) !=
				(int)USB_COMMAND_ROUTER_DISABLED_MODE)
				return USB_WRITE_INVALID;
			if ((integer_value != 0 && integer_value != 1) ||
				!RotorCalibrationService_SetReverse(integer_value != 0))
				return USB_DATA_OUT_OF_RANGE;
			return USB_NO_ERROR;
		case USB_CURRENT_CAL:
			return UsbCommandRouter_WriteParameter(
				MOTOR_PARAMETER_CALIBRATION_CURRENT_A, value);
		case USB_CURRENT_LIMIT:
			return UsbCommandRouter_WriteParameter(MOTOR_PARAMETER_CURRENT_LIMIT_A, value);
		case USB_SPEED_LIMIT:
			return UsbCommandRouter_WriteParameter(MOTOR_PARAMETER_SPEED_LIMIT_RAD_S,
		...
		case USB_SPEED_KP:
			return UsbCommandRouter_WriteParameter(MOTOR_PARAMETER_SPEED_KP, value);
		case USB_SPEED_KI:
			return UsbCommandRouter_WriteParameter(MOTOR_PARAMETER_SPEED_KI, value);
		case USB_POS_ACC:
			return UsbCommandRouter_WriteParameter(
		...
		case USB_CAN_BR:
			...
			return integer_value >= 0 &&
				CanConfigurationService_SetBitrateKbps((uint32_t)integer_value) ?
				USB_NO_ERROR : USB_DATA_OUT_OF_RANGE;
		case USB_CAN_HB:
			if (value_is_float != 0U)
				return USB_DATA_INVALID;
			return integer_value >= 0 &&
				CanConfigurationService_SetHeartbeatMs((uint32_t)integer_value) ?
				USB_NO_ERROR : USB_DATA_OUT_OF_RANGE;
		case USB_RS:
			return UsbCommandRouter_WriteParameter(
...
static UsbCommandError UsbCommandRouter_Read(
	...
	UsbParameterId parameter)
{
	...
	{
		case USB_MODE:
			snprintf(response->text, sizeof(response->text), "mode=%d\r\n", (int)UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_MODE, 1.0f)); break;
		case USB_CURRENT_SET:
			snprintf(response->text, sizeof(response->text), "i_set=%.2fA\r\n", UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_CURRENT_REFERENCE_A, 1.0f)); break;
		case USB_SPEED_SET:
			snprintf(response->text, sizeof(response->text), "spd_set=%.2fr/s\r\n", UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_SPEED_REFERENCE_RAD_S, USB_COMMAND_ROUTER_ONE_BY_2PI)); break;
		case USB_POS_SET:
			snprintf(response->text, sizeof(response->text), "pos_set=%.2fr\r\n", UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_POSITION_REFERENCE_RAD, USB_COMMAND_ROUTER_ONE_BY_2PI)); break;
		case USB_NODE_ID:
			snprintf(response->text, sizeof(response->text), "can_id=%u\r\n", (unsigned int)CanConfigurationService_GetNodeId()); break;
		case USB_POLEPARIS:
			snprintf(response->text, sizeof(response->text), "pol=%d\r\n", (int)UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_POLE_PAIRS, 1.0f)); break;
		case USB_ENCODER_STATE:
			snprintf(response->text, sizeof(response->text), "encoder=TLE5012B-%s.\r\n", UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_ENCODER_ONLINE, 1.0f) != 0.0f ? "Online" : "Offline"); break;
		case USB_ENCODER_REVERSE:
			snprintf(response->text, sizeof(response->text), "erv=%u\r\n", (unsigned int)UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_ENCODER_REVERSED, 1.0f)); break;
		case USB_CURRENT_CAL:
			snprintf(response->text, sizeof(response->text), "i_cal=%.2fA\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_CALIBRATION_CURRENT_A, 1.0f)); break;
		case USB_CURRENT_LIMIT:
			snprintf(response->text, sizeof(response->text), "i_lim=%.2fA\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_CURRENT_LIMIT_A, 1.0f)); break;
		case USB_SPEED_LIMIT:
			snprintf(response->text, sizeof(response->text), "spd_lim=%.2fr/s\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_SPEED_LIMIT_RAD_S, USB_COMMAND_ROUTER_ONE_BY_2PI)); break;
		case USB_SPEED_ACC:
			snprintf(response->text, sizeof(response->text), "spd_acc=%.2fr/s2\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_SPEED_ACCELERATION_RAD_S2, USB_COMMAND_ROUTER_ONE_BY_2PI)); break;
		case USB_SPEED_DEC:
			snprintf(response->text, sizeof(response->text), "spd_dec=%.2fr/s2\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_SPEED_DECELERATION_RAD_S2, USB_COMMAND_ROUTER_ONE_BY_2PI)); break;
		case USB_SPEED_KP:
			snprintf(response->text, sizeof(response->text), "spd_kp=%.2f\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_SPEED_KP, 1.0f)); break;
		case USB_SPEED_KI:
			snprintf(response->text, sizeof(response->text), "spd_ki=%.2f\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_SPEED_KI, 1.0f)); break;
		case USB_POS_ACC:
			snprintf(response->text, sizeof(response->text), "pos_acc=%.3fr/s2\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_POSITION_ACCELERATION_RAD_S2, USB_COMMAND_ROUTER_ONE_BY_2PI)); break;
		case USB_POS_DEC:
			snprintf(response->text, sizeof(response->text), "pos_dec=%.3fr/s2\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_POSITION_DECELERATION_RAD_S2, USB_COMMAND_ROUTER_ONE_BY_2PI)); break;
		case USB_POS_MAXSPEED:
			snprintf(response->text, sizeof(response->text), "pos_maxspd=%.3fr/s\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_POSITION_MAX_SPEED_RAD_S, USB_COMMAND_ROUTER_ONE_BY_2PI)); break;
		case USB_POS_KP:
			snprintf(response->text, sizeof(response->text), "pos_kp=%.2f\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_POSITION_KP_A_PER_RAD, 1.0f)); break;
		case USB_POS_KD:
			snprintf(response->text, sizeof(response->text), "pos_kd=%.2f\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_POSITION_KD_A_PER_RAD_S, 1.0f)); break;
		case USB_POS_KI:
			snprintf(response->text, sizeof(response->text), "pos_ki=%.3f\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_POSITION_KI_A_PER_RAD_S, 1.0f)); break;
		case USB_POS_INTEGRAL_LIMIT:
			snprintf(response->text, sizeof(response->text), "pos_i_limit=%.3fA\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_POSITION_INTEGRAL_LIMIT_A, 1.0f)); break;
		case USB_CASCADE_POS_KP:
			snprintf(response->text, sizeof(response->text), "cascade_pos_kp=%.3f\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_CASCADE_POSITION_KP_PER_S, 1.0f)); break;
		case USB_CASCADE_POS_KD:
			snprintf(response->text, sizeof(response->text), "cascade_pos_kd=%.3f\r\n", UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_CASCADE_POSITION_KD, 1.0f)); break;
		case USB_CAN_BR:
			snprintf(response->text, sizeof(response->text), "can_br=%ukbps\r\n", (unsigned int)CanConfigurationService_GetBitrateKbps()); break;
		case USB_CAN_HB:
			snprintf(response->text, sizeof(response->text), "can_hb=%ums\r\n", (unsigned int)CanConfigurationService_GetHeartbeatMs()); break;
		case USB_VBUS: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_BUS_VOLTAGE_V, 1.0f); snprintf(response->text, sizeof(response->text), "vbus=%.2fV\r\n", value); break;
		case USB_IBUS: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_BUS_CURRENT_A, 1.0f); snprintf(response->text, sizeof(response->text), "ibus=%.2fA\r\n", value); break;
		case USB_IA: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_PHASE_A_CURRENT_A, 1.0f); snprintf(response->text, sizeof(response->text), "ia=%.2fA\r\n", value); break;
		case USB_IB: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_PHASE_B_CURRENT_A, 1.0f); snprintf(response->text, sizeof(response->text), "ib=%.2fA\r\n", value); break;
		case USB_IC: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_PHASE_C_CURRENT_A, 1.0f); snprintf(response->text, sizeof(response->text), "ic=%.2fA\r\n", value); break;
		case USB_ID: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_D_AXIS_CURRENT_A, 1.0f); snprintf(response->text, sizeof(response->text), "id=%.2fA\r\n", value); break;
		case USB_IQ: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_Q_AXIS_CURRENT_A, 1.0f); snprintf(response->text, sizeof(response->text), "iq=%.2fA\r\n", value); break;
		case USB_SPEED2_FILT: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_MECHANICAL_SPEED_RAD_S, 1.0f); snprintf(response->text, sizeof(response->text), "spd2_filt=%.2fr/s\r\n", value); break;
		case USB_POS2_FILT: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_MECHANICAL_POSITION_RAD, 1.0f); snprintf(response->text, sizeof(response->text), "pos2_filt=%.3fr\r\n", value); break;
		case USB_TEMP: value = UsbCommandRouter_ReadTelemetry(MOTOR_TELEMETRY_TEMPERATURE_C, 1.0f); snprintf(response->text, sizeof(response->text), "temp=%.2fC\r\n", value); break;
		case USB_RS: value = UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_PHASE_RESISTANCE_OHM, 1000.0f); snprintf(response->text, sizeof(response->text), "Rs=%.2fmohm\r\n", value); break;
		case USB_LD: value = UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_D_AXIS_INDUCTANCE_H, 1000000.0f); snprintf(response->text, sizeof(response->text), "Ld=%.2fuH\r\n", value); break;
		case USB_LQ: value = UsbCommandRouter_ReadParameter(MOTOR_PARAMETER_Q_AXIS_INDUCTANCE_H, 1000000.0f); snprintf(response->text, sizeof(response->text), "Lq=%.2fuH\r\n", value); break;
	...
	const UsbCommandRouterState *state, UsbCommandRouterResponse *response)
{
	...
	if (command == NULL || state == NULL || response == NULL ||
	...
	{
		result = UsbCommandRouter_Write(parameter_id, command->value,
	...
	{
		result = UsbCommandRouter_Read(state, response, parameter_id);

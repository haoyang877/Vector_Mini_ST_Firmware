...
static void Encoder_MarkReadStatus(EncoderContext *encoder,
...
static uint16_t Encoder_ApplyDirectionQ15(const EncoderContext *encoder,
	uint16_t raw_q15)
{
...
static uint16_t Encoder_ApplyLinearizationQ15(const EncoderContext *encoder,
	uint16_t raw_q15)
{
	...
	int32_t correction_b = encoder->linearization_lut_q15[
		(lut_index + 1U) & (ENCODER_OFFSET_LUT_SIZE - 1U)];
	int32_t correction = correction_a +
		(((correction_b - correction_a) * fraction) >> 6);
...
{
	memset(encoder->velocity_delta_history, 0,
		sizeof(encoder->velocity_delta_history));
...
void Encoder_SetReverse(EncoderContext *encoder, bool reverse)
{
	...
	memset(encoder->linearization_lut_q15, 0,
...
static void Encoder_UpdateVelocity2kHz(EncoderContext *encoder,
	uint32_t pole_pairs)
{
	int64_t delta64;
	int32_t delta_q15;
	...
	encoder->velocity_delta_sum -=
		encoder->velocity_delta_history[encoder->velocity_history_index];
	encoder->velocity_delta_history[encoder->velocity_history_index] = delta_q15;
	encoder->velocity_delta_sum += delta_q15;
	encoder->velocity_history_index = (uint8_t)(
		(encoder->velocity_history_index + 1U) % ENCODER_VELOCITY_WINDOW);
	if (encoder->velocity_sample_count < ENCODER_VELOCITY_WINDOW)
		encoder->velocity_sample_count++;
	encoder->velocity_ready =
		encoder->velocity_sample_count == ENCODER_VELOCITY_WINDOW;

	sum_abs = encoder->velocity_delta_sum;
	if (sum_abs < 0)
		sum_abs = -sum_abs;
	if (!encoder->velocity_ready ||
		sum_abs <= ENCODER_VELOCITY_ZERO_THRESHOLD_Q15)
	{
		encoder->vel_mech = 0.0f;
	}
	else
	{
		velocity_scale = ENCODER_TWO_PI / ((float)ENCODER_Q15_CPR *
			(float)ENCODER_VELOCITY_WINDOW *
			encoder->velocity_sample_period_s);
		encoder->vel_mech =
			(float)encoder->velocity_delta_sum * velocity_scale;
	}
...
static void Encoder_UpdateAngles(EncoderContext *encoder, uint32_t pole_pairs)
{
	...
	electrical_q15 = (uint16_t)((uint32_t)(uint16_t)(
		encoder->linearized_q15 - encoder->electrical_zero_q15) * pole_pairs);
	...
	float velocity_sample_period_s)
{
	...
	encoder->shadow_q15 = 0;
	encoder->mechanical_zero_shadow_q15 =
		(int64_t)encoder->mechanical_zero_q15;
	...
	const EncoderSample *sample)
{
	...
	if (sample->status != ENCODER_READ_OK)
	{
		Encoder_MarkReadStatus(encoder, sample->status);
		return;
	}
	...
	delta_q15 = (int32_t)linearized_q15 -
		(int32_t)encoder->previous_linearized_q15;

...
{
    return config != NULL && isfinite(config->target_position) &&
           ...
           config->maximum_speed <= config->speed_limit && isfinite(config->kp) &&
           config->kp >= 0.0f && config->kp <= POSITION_IMPEDANCE_KP_MAX_A_PER_RAD &&
           isfinite(config->kd) && config->kd >= 0.0f &&
           config->kd <= POSITION_IMPEDANCE_KD_MAX_A_PER_RAD_S && isfinite(config->ki) &&
           config->ki >= 0.0f && config->ki <= POSITION_IMPEDANCE_KI_MAX_A_PER_RAD_S &&
           isfinite(config->integral_limit) && config->integral_limit >= 0.0f &&
           config->integral_limit <= CURRENT_SENSE_PROFILE_COMMAND_LIMIT_MAX_A &&
           ...
           config->friction_positive_current <= CURRENT_SENSE_PROFILE_COMMAND_LIMIT_MAX_A &&
           isfinite(config->friction_negative_current) &&
           config->friction_negative_current >= 0.0f &&
           config->friction_negative_current <= CURRENT_SENSE_PROFILE_COMMAND_LIMIT_MAX_A &&
           isfinite(config->breakaway_positive_current) &&
           config->breakaway_positive_current >= config->friction_positive_current &&
           config->breakaway_positive_current <= CURRENT_SENSE_PROFILE_COMMAND_LIMIT_MAX_A &&
           isfinite(config->breakaway_negative_current) &&
           config->breakaway_negative_current >= config->friction_negative_current &&
           config->breakaway_negative_current <= CURRENT_SENSE_PROFILE_COMMAND_LIMIT_MAX_A &&
           ...
           config->friction_release_slew_rate > 0.0f && isfinite(config->friction_position_enter) &&
           config->friction_position_enter > 0.0f && isfinite(config->friction_position_exit) &&
...
static float PositionImpedance_GetFrictionMagnitude(const PositionImpedanceConfig_TypeDef *config,
                                                    int8_t direction,
                                                    bool breakaway)
{
    if (direction > 0)
        return breakaway ? config->breakaway_positive_current : config->friction_positive_current;
    if (direction < 0)
        return breakaway ? config->breakaway_negative_current : config->friction_negative_current;
    return 0.0f;
}
...
static void PositionImpedance_UpdateFriction(const PositionImpedanceConfig_TypeDef *config,
                                             float target_error)
{
    ...
    bool reference_is_moving = fast_abs(state.speed_reference) > config->friction_reference_speed;
    bool feedback_is_stopped = fast_abs(state.velocity_filtered) <= config->friction_stop_speed;
    bool trajectory_is_finished = fast_abs(config->target_position - state.position_reference) <=
    ...
    case POSITION_IMPEDANCE_FRICTION_TRACK:
        ...
        else if (desired_direction == 0 && fast_abs(target_error) >= config->friction_position_exit)
        ...
        braking_distance =
            speed_toward_target > 0.0f
                ? (speed_toward_target * speed_toward_target) / (2.0f * config->deceleration)
                : 0.0f;
        ...
        friction_target = PositionImpedance_GetFrictionMagnitude(
            config, desired_direction, state.friction_breakaway_active);
    ...
    case POSITION_IMPEDANCE_FRICTION_LANDING:
        ...
        else
        {
            ...
            speed_toward_target = (float)state.friction_direction * state.velocity_filtered;
            reference_speed_toward_target = (float)state.friction_direction * state.speed_reference;
            tracking_error_toward_target =
                (float)state.friction_direction * (state.position_reference - measured_position);
            speed_excess = speed_toward_target - fast_max(reference_speed_toward_target, 0.0f);
            landing_scale = 0.0f;
            landing_span =
                state.friction_recovery_landing_start_distance - config->friction_landing_position;

            if (state.friction_direction != 0 && remaining_distance > 0.0f && landing_span > 0.0f)
            {
                landing_scale = constrain((remaining_distance - config->friction_landing_position) /
                                              landing_span,
                                          0.0f,
                                          1.0f);
                     ...
                     remaining_distance > 0.0f)
            {
                ...
                {
                    landing_scale =
                        constrain(tracking_error_toward_target / config->friction_landing_position,
                                  0.0f,
                                  1.0f);
                    if (speed_excess > 0.0f)
                    {
                        landing_speed_scale =
                            constrain(1.0f - speed_excess /
            ...
            {
                landing_magnitude =
                    PositionImpedance_GetFrictionMagnitude(config, state.friction_direction, false);
                ...
                state.friction_recovery_cooldown_counter == 0U)
            {
                    ...
                    state.friction_recovery_delay_counter >= recovery_delay_ticks)
                {
                    ...
                    friction_target = PositionImpedance_GetFrictionMagnitude(
                        config, state.friction_direction, true);
                    if (state.friction_direction < 0)
                        friction_target = -friction_target;
                }
            }
    ...
    case POSITION_IMPEDANCE_FRICTION_HOLD:
        ...
        {
            state.friction_state = POSITION_IMPEDANCE_FRICTION_LANDING;
            state.friction_direction = fast_abs(target_error) > config->friction_position_enter
    ...
    case POSITION_IMPEDANCE_FRICTION_RECOVERY:
        recovery_magnitude =
            PositionImpedance_GetFrictionMagnitude(config, state.friction_direction, true);
        friction_target = state.friction_direction > 0 ? recovery_magnitude : -recovery_magnitude;
        recovery_displacement = (float)state.friction_direction *
                                (measured_position - state.friction_recovery_start_position);
        if (state.friction_direction == 0 ||
            (float)state.friction_direction * target_error <= config->friction_position_enter)
        {
            state.friction_state = POSITION_IMPEDANCE_FRICTION_LANDING;
            state.friction_recovery_landing_start_distance = 0.0f;
            state.friction_recovery_pulse_counter = 0U;
        ...
        else if (recovery_displacement >= config->friction_position_exit)
        {
            ...
            state.friction_recovery_landing_start_distance = fast_max(
                (float)state.friction_direction * target_error, config->friction_landing_position);
            state.friction_recovery_pulse_counter = 0U;
            state.friction_recovery_cooldown_counter = recovery_cooldown_ticks;
            friction_target =
                PositionImpedance_GetFrictionMagnitude(config, state.friction_direction, false);
    ...
    case POSITION_IMPEDANCE_FRICTION_DIRECTION_CHANGE:
                 ...
                 -POSITION_IMPEDANCE_INTEGRAL_ZERO_THRESHOLD_A))
        {
            ...
            state.friction_pending_direction = 0;
            state.friction_recovery_landing_start_distance = 0.0f;
            state.friction_breakaway_active = feedback_is_stopped && state.friction_direction != 0;
            state.friction_breakaway_start_position = measured_position;
        }
        break;
    ...
    state.friction_current +=
        constrain(friction_target - state.friction_current, -friction_step, friction_step);
    state.friction_current =
        constrain(state.friction_current, -config->output_limit, config->output_limit);
                              ...
                              PositionImpedanceOutput_TypeDef *output)
{
    ...
    if (output == NULL || !isfinite(measured_position) || !PositionImpedance_ConfigIsValid(config))
        return false;
    ...
    if (!state.initialized)
    {
        ...
        target_direction = 0;
        if (fast_abs(config->target_position - measured_position) > config->position_error_window)
    ...
    {
        trajectory_needs_update = state.last_target != config->target_position ||
                                  state.last_acceleration != config->acceleration ||
                                  state.last_deceleration != config->deceleration ||
                                  state.last_maximum_speed != config->maximum_speed;
        if (state.last_target != config->target_position)
        {
            ...
            {
                state.friction_state = target_direction == 0 ? POSITION_IMPEDANCE_FRICTION_LANDING
    ...
    {
        TRAJ_plan(config->target_position,
    ...
    state.velocity_filtered += velocity_filter_alpha * (raw_velocity - state.velocity_filtered);
    ...
    if (config->friction_feedforward_enabled)
        hold_exit_position = fast_max(hold_exit_position, config->friction_position_exit);
    ...
    friction_allows_integral =
        !config->friction_feedforward_enabled ||
        (state.friction_state == POSITION_IMPEDANCE_FRICTION_LANDING &&
         fast_abs(config->target_position - state.position_reference) <=
             config->position_error_window &&
         fast_abs(state.friction_current) <= POSITION_IMPEDANCE_INTEGRAL_FRICTION_MAX_A &&
         fast_abs(state.velocity_filtered) <= config->friction_stop_speed &&
         fast_abs(target_error) <= config->friction_landing_position);
    ...
    {
        integral_decay_step =
            constrain(state.integral * POSITION_IMPEDANCE_INTEGRAL_OPPOSING_DECAY_RATIO_PER_TICK,
    ...
    else
    {
        ...
        integral_opposes_motion = target_is_unsettled && state.integral * desired_motion < 0.0f;
        feedback_follows_motion =
            target_is_unsettled &&
            fast_abs(state.velocity_filtered) > POSITION_IMPEDANCE_INTEGRAL_DECAY_MIN_SPEED_RAD_S &&
            desired_motion * state.velocity_filtered > 0.0f && desired_motion * raw_velocity > 0.0f;

        if (state.integral_transport_active && (trajectory_is_moving || target_is_unsettled) &&
            (integral_opposes_motion || feedback_follows_motion))
        {
            ...
            else
            {
                ...
                integral_decay_ratio = constrain(
                    integral_transport_distance / POSITION_IMPEDANCE_INTEGRAL_DECAY_DISTANCE_RAD,
                    0.0f,
        ...
        if (!trajectory_held_for_friction && friction_allows_integral && !state.target_reached &&
            fast_abs(state.speed_reference) <= POSITION_IMPEDANCE_INTEGRAL_SPEED_RAD_S &&
        ...
        {
            integral_candidate =
                constrain(state.integral + config->ki * position_error * Position_Ts,
                          -integral_limit,
                          integral_limit);
    ...
    current_candidate =
        proportional_current + damping_current + state.integral + state.friction_current;
    if (!isfinite(current_candidate))
        return false;
    state.iq_reference = constrain(current_candidate, -config->output_limit, config->output_limit);
    PositionImpedance_CopyOutput(output);
    return true;
}

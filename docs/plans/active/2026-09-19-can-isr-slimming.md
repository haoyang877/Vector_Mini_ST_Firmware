...
| D5-b | 前台消费点 | **主循环入口**（`main.c` 循环首部，`CAN_SendMessage()` 之前）。 |
...
- 实施入口：`can_command_binding.c`（改为入队）、`interface_can.{c,h}`（新增
  `CAN_ServiceCommands` 与 `CAN_GetQueueDropCount` 契约）、`main.c`（循环首部调用）、
  夹具 `run_can_status_tests.py`（新增队列用例）与差分夹具。

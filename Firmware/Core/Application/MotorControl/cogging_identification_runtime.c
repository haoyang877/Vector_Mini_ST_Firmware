#include "Core/Application/MotorControl/cogging_identification_runtime.h"
...
static float SpeedTolerance(const MotorControlContext *motor, float target)
{
	return FastMath_Max(0.05f,
		FastMath_Abs(target) * motor->commissioning_tuning->cogging.
	...
	EncoderContext *encoder, const MotorControlContext *motor)
{
	...
	uint16_t minimum_samples = motor->commissioning_tuning->cogging.
	...
	else
	{
		...
		int32_t limit_ma = (int32_t)(motor->commissioning_tuning->cogging.
			maximum_current_a * 1000.0f);
	...
	MotorStateContext *motor_state)
{
	...
	if (context == 0 || motion == 0 || current_control == 0 || motor == 0 ||
		speed_controller == 0 || encoder == 0 || motor->commissioning_tuning == 0 ||
	...
	speed = motor->commissioning_tuning->cogging.speed_rad_s;
	required_travel_q15 = (int64_t)motor->commissioning_tuning->cogging.turns *
		(int64_t)ENCODER_Q15_CPR;
	if (speed <= 0.0f || required_travel_q15 <= 0 ||
		motor->commissioning_tuning->cogging.minimum_samples_per_bin == 0U)
	{
		...
		context->state == COGGING_IDENT_SAMPLE_CCW)
	{
		...
		context->stage_ticks++;
		if (context->stage_ticks >= (uint32_t)(motor->commissioning_tuning->
			cogging.stage_timeout_s / motor->schedule.current_period_s))
		{
			MotorState_RaiseFault(motor_state, MOTOR_FAULT_COGGING_IDENTIFICATION);
			return;
		}
		if (waiting)
		{
			...
			if (context->stable_ticks >= (uint32_t)(motor->commissioning_tuning->
				cogging.stable_time_s / motor->schedule.current_period_s))
			{

...
static unsigned low_state_calls;
static void PWM_Outputs_Low(void) { low_state_calls++; }
...
int main(void) {
    ...
    assert(low_state_calls == 1);
    assert(adc_irq_mask == ADC_IT_JEOS);
    puts("PASS actual board startup: unconfigured phase outputs held low, sampling and communication retained");
    return 0;
}
'''

...
/* CAN 状态/接收端口：状态帧入队与接收取帧的 HAL 适配。
 * 控制路径（滤波启动、波特率切换、应答发送）见 comm_control_stm32g4.c。 */
...
bool comm_hw_can_try_send_status(uint16_t identifier, const uint8_t *data, size_t length)
{
    ...
    if (identifier > 0x7FFU || data == NULL || length != 48U)
    {
        return false;
    }
    ...
    if (hfdcan1.Init.TxFifoQueueMode != FDCAN_TX_QUEUE_OPERATION ||
        HAL_FDCAN_IsTxBufferMessagePending(
            &hfdcan1, FDCAN_TX_BUFFER0 | FDCAN_TX_BUFFER1 | FDCAN_TX_BUFFER2) != 0U)
    {
        return false;
    }

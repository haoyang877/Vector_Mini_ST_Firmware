#include "Core/Application/MotorControl/motor_service_adapter.h"
	...
	MotorConfigurationAdapterContext *adapter, MotorParameterId parameter)
{
	...
	float bandwidth = adapter->current_loop_bandwidth_rad_s;
	...
	float current_loop_bandwidth_rad_s, MotorStateContext *motor_state)
{
	...
	if (context == 0 || motor == 0 || critical_section == 0 ||
		current_loop_bandwidth_rad_s <= 0.0f ||
		critical_section->enter == 0 || critical_section->exit == 0 ||
		motor_state == 0)
		return port;
	context->motor = motor;
	context->motor_state = motor_state;
	context->current_loop_bandwidth_rad_s = current_loop_bandwidth_rad_s;

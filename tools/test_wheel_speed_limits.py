"""Host regression of production parameter loading and CAN/USB limit cases."""
import argparse
...
def command_case(path, name):
    ...
    source = function_source(source, "CAN_ReceiveMessage_Update" if name.startswith("CAN_")
                             else "USB_ReceiveMessage_Update")
...
'''
    utils = (ROOT / "System/utils.c").read_text(encoding="utf-8")
    prelude += "\n".join(function_source(utils, n) for n in ("constrain", "fast_min"))
    production = (ROOT / "Foc/foc_param.c").read_text(encoding="utf-8")
    prelude += re.sub(r'^#include.*$', '', production, flags=re.M)
    prelude += '\nstatic void can_set(int id, float data) { int data_int=(int)data; switch(id) {\n'
    prelude += '\n'.join(command_case("Communication/interface_can.c", n) for n in
                         ("CAN_SET_NODE_ID", "CAN_SET_SPEED_LIMIT"))
    prelude += '\n} }\nstatic int usb_set(int id, float data) { int data_int=(int)data; int int_or_float=0; switch(id) {\n'
    prelude += '\n'.join(command_case("Communication/interface_usb.c", n) for n in
                         ("USB_NODE_ID", "USB_SPEED_LIMIT"))
    return prelude + '\n} return USB_NO_ERROR; }\n' + r'''
int main(void) {
    ...
    for(node=1; node<=2; node++) {
        ...
        assert(usb_set(USB_SPEED_LIMIT,20/_2PI)==USB_NO_ERROR);
        assert(MotorControl.speed_limit==20);
        assert(usb_set(USB_SPEED_LIMIT,3.184f)==USB_DATA_OUT_OF_RANGE);
        MotorControl.speedRef=-20;
        assert(Param_SetSpeedLimit(10) && MotorControl.speedRef==-10);
    }
    puts("PASS CAN rad/s and USB r/s boundaries; reduced limits clamp both command signs");
    ...
    assert(usb_set(USB_NODE_ID,4)==USB_NO_ERROR);
    assert(CANMsg.node_id==4 && MotorControl.speed_limit==.5f*_2PI && MotorControl.speedRef==-.5f*_2PI);
    assert(usb_set(USB_SPEED_LIMIT,1)==USB_DATA_OUT_OF_RANGE);
...
def main():
    ...
    includes = [str(out)] + [str(ROOT / p) for p in ("System", "Foc", "Bsp", "Communication")]
    command = cc + ["-std=c99", "-O1", "-UNDEBUG", "-Wall", "-Wextra", "-Werror"]
    command += [item for p in includes for item in ("-I", p)]
    command += [str(source), str(ROOT / "software/config/motor_axis_profile.c"), "-o", str(exe)]
    logs = []

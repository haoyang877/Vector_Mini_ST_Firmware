#!/usr/bin/env python3
"""Run and review the Vector Mini ST bidirectional friction identification."""
...
STATE_NAMES = {0: "idle", 1: "tracking", 2: "sampling", 3: "stopping", 4: "complete", 5: "failed"}
REASON_NAMES = {
    0: "none", 1: "cancelled", 2: "invalid configuration",
    3: "speed tracking timeout", 4: "sampling timeout", 5: "stop timeout",
    6: "current saturation", 7: "invalid sample", 8: "fit rejected",
...
class VectorUsb:
    ...
    def command(self, command: str) -> str:
        ...
        if response in {"Syntax error!", "Write invalid!", "Unknowned parameter!",
                        "Data invalid!", "Data out of range!"}:
    ...
    def read_export(self) -> list[Sample]:
        ...
        while True:
            ...
            samples.append(Sample(int(item.group(1)), float(item.group(2)),
                                  float(item.group(3)), float(item.group(4)),
                                  int(item.group(5))))
        if len(samples) != count or count != EXPECTED_POINTS:
            raise ProtocolError(f"Expected {EXPECTED_POINTS} samples, received {len(samples)}")
        if sorted(sample.index for sample in samples) != list(range(count)):
            raise ProtocolError("Friction export has missing or duplicate sample indices")
...
def parse_status(response: str) -> dict[str, float]:
    return {field: extract_number(response, field)
...
def constrained_fit(samples: list[Sample], positive: bool) -> Fit:
    ...
    if any(value <= 0.0 or not math.isfinite(value) for value in x + y):
        raise ProtocolError("The measured current does not oppose motion in every sample")
    count = float(len(x))
    ...
    if slope < 0.0:
        slope, intercept = 0.0, sum_y / count
    if intercept < 0.0:
        intercept, slope = 0.0, sum_xy / sum_xx
    rmse = math.sqrt(sum((yv - (intercept + slope * xv)) ** 2
                         for xv, yv in zip(x, y)) / count)
...
def preflight(device: VectorUsb) -> None:
    ...
    if mode != 0 or error != 0 or "Online" not in encoder or speed_limit < REQUIRED_SPEED_LIMIT_RPS:
        raise ProtocolError(
            f"Preflight failed: mode={mode}, error={error}, encoder={encoder}, "
            f"speed_limit={speed_limit:.3f} rev/s"
        )
...
def candidate_values(device: VectorUsb) -> dict[str, float]:
    ...
    return {name: extract_number(device.read(command), response_field)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not args.yes and input(
                f"Confirm continuous bidirectional rotation is safe at up to "
                f"{REQUIRED_SPEED_LIMIT_RPS:.1f} rev/s; type RUN: ") != "RUN":
            return 2
        ...
        while time.monotonic() < deadline:
            ...
            state, reason = int(status["friction_state"]), int(status["reason"])
            summary = (f"state={STATE_NAMES.get(state, state)}, point="
            ...
            if state == STATE_FAILED:
                running = False
                raise ProtocolError(f"Identification failed: {REASON_NAMES.get(reason, reason)}")
        ...
        host_pos = constrained_fit(samples, True)
        host_neg = constrained_fit(samples, False)
        host = {
            "coulomb_pos_a": host_pos.coulomb_a,
            "coulomb_neg_a": host_neg.coulomb_a,
            "viscous_pos_a_per_rad_s": host_pos.viscous_a_per_rad_s,
            "viscous_neg_a_per_rad_s": host_neg.viscous_a_per_rad_s,
            "rmse_pos_a": host_pos.rmse_a,
            "rmse_neg_a": host_neg.rmse_a,
        }
        for name, value in host.items():
            if not math.isclose(value, firmware[name], rel_tol=2e-3, abs_tol=2e-5):
                raise ProtocolError(f"Host/firmware fit mismatch for {name}")
        print("\nCandidate current-domain model:")
        print(f"  positive: Iq = {host['coulomb_pos_a']:.6f} + "
              f"{host['viscous_pos_a_per_rad_s']:.6f} * speed(rad/s) A")
        print(f"  negative: Iq = -{host['coulomb_neg_a']:.6f} + "
              f"{host['viscous_neg_a_per_rad_s']:.6f} * speed(rad/s) A")
        print(f"  RMSE: +{host['rmse_pos_a']:.6f} A, "
              f"-{host['rmse_neg_a']:.6f} A")
        print(f"  torque estimate (Kt={torque_constant_nm_per_a:.6f} N*m/A): "
        ...
        payload = {
            ...
            "torque_conversion": {
                ...
                "viscous_pos_nm_per_rad_s":
                    host["viscous_pos_a_per_rad_s"] * torque_constant_nm_per_a,
                "viscous_neg_nm_per_rad_s":
                    host["viscous_neg_a_per_rad_s"] * torque_constant_nm_per_a,
            },
        ...
        if args.apply:
            if args.yes or input("Type APPLY to copy the candidate into RAM: ") == "APPLY":
                device.write("fap", 1)
                ...
                if args.save:
                    ...
                    while time.monotonic() < save_deadline:
                        ...
                        raise ProtocolError(
                            "Controller did not return to Disable after saving")
                    if int(extract_number(device.read("fva"),
                                          "friction_model_valid")) != 1:
                        raise ProtocolError(
                            "Applied friction model is not valid after saving")
                    payload["saved"] = True
        if args.output:
        ...
        if running and device is not None:
            try:
                device.write("mod", 0)
            except Exception:
                pass
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run bidirectional friction identification")

...
- **时序**：RX 中断处理时延、TX 重试上限（5 次）、2 kHz 波特率切换与心跳判据、
...
- **决策点 D3（S5）——已裁决（2026-09-19，用户）：执行中断外移**，但按约定先完成最坏时延
  分析与设计，代码需台架授权；分析与设计见
  [CAN 接收中断瘦身（S5）](2026-09-19-can-isr-slimming.md)（含优先级/原子性证据与 D5-a..d）。
  以下为原自评内容，保留供对照（其结论"建议维持现状"已被用户裁决取代）：事实：flash 写期间 `__disable_irq()`
...
| S5 | 裁决记录（如执行） | ✅ 已立项：用户裁决执行；分析与设计见 [CAN 接收中断瘦身](2026-09-19-can-isr-slimming.md)，代码待台架授权 |
...
- 用户裁决：**执行** RX 中断外移（覆盖 §8 D3 的"建议维持现状"）；约定先出分析再动代码。
- 本会话产出 [CAN 接收中断瘦身（S5）设计与验收计划](2026-09-19-can-isr-slimming.md)：
  实测中断优先级（ADC 0 / CAN 1 / TIM7 3 / DMA 5）、ISR 工作量构成、主循环最坏项
  （含 Flash 保存期 `__disable_irq()`）、以及"命令对快环原子性依赖 prio 0/1 关系"这一
  必须用临界区补偿的约束；列出会变化的可观测面、离线/实机验收与 D5-a..d 待裁决项。
- 未改动任何固件代码。
- 后续裁决（2026-09-19）：**S5 代码等实机台架后再写**（避免提交无法验证的控制时序改动）；
  D5-d = 容量 8 + 丢弃最新 + 计数、D4 = 暂不收紧；D5-a（是否保留模式/禁用类中断内快速路径）
  未决，实施前需二选一确认。详见 S5 文档 §8。
- 用户规格澄清（2026-09-19，覆盖 D5-b）：通信时序为"中断只把帧存入 RX ring → **2 kHz 中断**
  从 RX ring 取数据派发 → 组包入 TX ring → TX ring 同样按 2 kHz 取出并发送"。据此
  D5-b 由"主循环入口"更正为"2 kHz 时基"，并把 **TX ring 与 2 kHz 发送节拍**一并纳入 S5 范围
  （原 S5 只覆盖 RX 瘦身）；同时记录现状单槽 TX 会丢突发应答、需在新设计中修复并回归。
- **2026-09-20 更新：S5 已按方案 A 实施并离线验证（PR 档全绿）**；实现与证据见
  [S5 计划](2026-09-19-can-isr-slimming.md) §10；实机 CAN 回归待台架。

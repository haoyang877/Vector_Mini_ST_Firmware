"""运行离线 Python 测试与可选原生 C 检查；不访问任何硬件。"""
...
NATIVE = (
    ...
    "test_current_oversampling",
    "test_current_precision",
    "test_adc_fast_dispatch",
...
def main():
    ...
    parser.add_argument(
        "--reference-source", type=Path, help="Pre-change position_cascade.c for equivalence checks"
    )
        ...
        for name in NATIVE:
            commands.append(
                (
                    ...
                    [
                        sys.executable,
                        str(ROOT / "tests/unit/native" / (name + ".py")),
                        "--cc",
                        args.cc,
                        "--out",
                        str(out / name),
        ...
        if args.reference_source:
            commands.append(
                (
                    ...
                    [
                        sys.executable,
                        str(ROOT / "tests/unit/native/test_position_core_equivalence.py"),
                        "--cc",
                        args.cc,
                        "--reference-source",
                        str(args.reference_source.resolve()),
                        "--out",
                        str(out / "equivalence"),
                    ],
                )
    ...
    for name, command in commands:
        ...
        print(
            f"{name}: {'PASS' if result.returncode == 0 else 'FAIL'} (exit {result.returncode})",
            flush=True,
        )

...
- 用户规格澄清（2026-09-19，覆盖 D5-b）：通信时序为"中断只把帧存入 RX ring → **1 kHz 中断**
  从 RX ring 取数据派发 → 组包入 TX ring → TX ring 同样按 1 kHz 取出并发送"。据此
  D5-b 由"主循环入口"更正为"1 kHz 时基"，并把 **TX ring 与 1 kHz 发送节拍**一并纳入 S5 范围

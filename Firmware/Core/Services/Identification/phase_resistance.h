#ifndef CORE_SERVICES_IDENTIFICATION_PHASE_RESISTANCE_H
#define CORE_SERVICES_IDENTIFICATION_PHASE_RESISTANCE_H
 ...
 * 5. 本模块不访问 CurrentControl、MotorControl、PWM、编码器、RTT 或通信资源。
 *
 * 调用顺序：Start() 开始一次测试；每个 CurrentControl 周期先调用 GetCommand() 取得
...
} PhaseResistanceConfig;

/** One completed CurrentControl control-cycle measurement supplied by the adapter. */
typedef struct
{
	float id;
	float iq;
	...
	float vd;
	float vq;
} PhaseResistanceSample;

/** Current-vector command returned to the CurrentControl adapter for the next cycle. */
typedef struct
{
	bool inject_current;
...
typedef struct
{
	...
	bool warning;
	bool balanced;
} PhaseResistanceResult;

/**
 * Per-measurement state storage. The CurrentControl adapter owns one instance and must
...
/**
 * Get the d/q current and electrical-angle command for the current CurrentControl cycle.

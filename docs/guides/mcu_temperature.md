...
启动阶段进行一次采样，之后1 kHz监督任务在JEOS后读取完整两通道结果、清标志并发起下一次转换。没有新增ADC1中断，也不延长ADC2电流采样或20 kHz FOC中断。一次序列约30.7 µs，在两次监督调用之间完成。
...
显示采用系数0.02的一阶低通，首次有效值直接初始化；过温判定使用未滤波温度，不等待低通响应。`McuTemperature`调试结构给出原始ADC、VDDA、未滤波温度、有效标志、失效毫秒数及累计有效样本数。

## 保护与兼容性

芯片温度90°C触发原有`High_Temprature`（故障10）。此阈值比数据手册最低温度等级的105°C结温上限低15°C，给校准公差、测量误差和热延迟留出余量；它是本版本保守软件阈值，尚未进行实物热箱验证，不表示整机热安全验收。

原始ADC为0/满量程、工厂常数无效、VDDA不在1.62～3.6 V或换算温度超出-40～150°C时拒绝读数并输出NaN。连续无有效样本100 ms（包括JEOS停止更新）触发新增`TemperatureSensor_Error`（故障17）。已有故障保持，不由温度覆盖；恢复有效读数不会自动清除已锁存故障。读数短时不更新可保持最近有效值，最多99 ms。
...
原生测试执行生产换算函数及监督任务，覆盖30/130°C两点、供电修正、负温、无效ADC/ROM、滤波与原始温度保护、100 ms超时、启动无ADC以及故障保持。普通/HIL构建和全套回归通过。静止实机验证与固件校验值见[报告](../reports/2026-09/mcu_temperature_20260918.md)。

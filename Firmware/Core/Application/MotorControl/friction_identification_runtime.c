#include "Core/Application/MotorControl/friction_identification_runtime.h"
...
static bool BuildConfig(FrictionIdentificationConfig *config,
	const EncoderContext *encoder,
	const ProductFrictionIdentificationConfig *profile)
{
	uint32_t index;
	if (config == 0 || encoder == 0 || profile == 0 ||
		profile->speed_point_count == 0U ||
		profile->speed_point_count >
	...
	config->speed_point_count = profile->speed_point_count;
	for (index = 0U; index < config->speed_point_count; index++)
		config->speed_points_rad_s[index] =
			profile->speed_points_rad_s[index];
	config->stable_time_s = profile->stable_time_s;
	config->track_timeout_s = profile->track_timeout_s;
	config->sample_timeout_s = profile->sample_timeout_s;
	config->stop_hold_time_s = profile->stop_hold_time_s;
	config->stop_timeout_s = profile->stop_timeout_s;
	config->speed_tolerance_ratio = profile->speed_tolerance_ratio;
	config->minimum_speed_tolerance_rad_s =
		profile->minimum_speed_tolerance_rad_s;
	config->stop_speed_rad_s = profile->stop_speed_rad_s;
	config->sample_turns = profile->sample_turns;
	config->minimum_sample_time_s = profile->minimum_sample_time_s;
	config->saturation_time_s = profile->saturation_time_s;
	config->rmse_floor_a = profile->rmse_floor_a;
	config->rmse_ratio_max = profile->rmse_ratio_max;
	return true;
}
	...
	const ProductFrictionIdentificationConfig *profile)
{
	...
	if (motor == 0 || encoder == 0 || profile == 0 ||
		...
		FRICTION_IDENTIFICATION_MAX_SPEED_POINTS)
		return false;
	last_index = profile->speed_point_count - 1U;
	return isfinite(motor->configuration.current_limit_a) &&
		...
		motor->configuration.speed_limit_rad_s >=
			profile->speed_points_rad_s[last_index] &&
...
FrictionIdentificationState FrictionIdentificationRuntime_Run(
	...
	const ProductFrictionIdentificationConfig *tuning)
{
	...
	if (context == 0 || motion == 0 || current_control == 0 || motor == 0 ||
		speed_controller == 0 || encoder == 0 || tuning == 0)
		return FRICTION_IDENT_FAILED;
	if (!RuntimeConfigIsValid(motor, encoder, tuning))
	{
	...
	if (!context->started)
	{
		if (!BuildConfig(&config, encoder, tuning) ||
